...
;;                   ;; Move standard libraries to the end of the list.
;;                   (dolist (lib '("stddebug.dg" "stdlib.dg"))
;;                     (when (member lib files)
;;                       (setq files (append (delete lib files) (list lib)))))
...
;; Any prompts which originate from the standard library are expected to be
;; handled by extensions which are loaded during game development.  For example,
;; to skip any yes/no prompts without waiting for input, define a new rule for
;; the (yesno) predicate:

;;   (yesno)	%% Automatically accept yes/no prompts.
...
;;     ;; Configure indentation, fill-column, and page-delimiter to match the
;;     ;; standard library.
;;     (add-hook 'dialog-mode-hook
;;               (lambda ()
;;                 (indent-tabs-mode)
